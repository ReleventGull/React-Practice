...
apiRouter.post('/users/signup', async (req, res, next) => {
    ...
    try {
    const checkForExistingUser = allUsers.filter(users => users.username == username)
    console.log("Existing user here", checkForExistingUser)
    if(checkForExistingUser.length >= 1) {
        res.send({error:"Invalid Credentials", name:"A user by that username already exists"})
    }else {
        const createdUser = await createUser(req.body)
        const token = jwt.sign({username: createdUser.username}, JWT_SECRET)
        res.send({message:"User created!", user:createdUser, user: createdUser, token: token })
    }
    }catch(error) {
...
apiRouter.post('/users/login', async(req, res, next) => {
    ...
    try {
        ...
        if(password == foundUser.password) {
            ...
            console.log(token)
            res.send({Message:"You're successfully logged in!", User: foundUser, token: token})
        }else {
